...
function Cart(props) {
    ...
    return (
        <Transition in={show} timeout={100}>
            {(state) => (
                <div className={`z-50 fixed inset-0 sm:left-1/3 md:left-1/2 xl:left-2/3 bg-white border px-4 py-6 transition-transform transform ease-in-out duration-700 ${state === 'entered' ? 'translate-x-0 ' : 'translate-x-full '}`}>
