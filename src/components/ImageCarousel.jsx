    ...
    return <>
        {
            <div ref={containerRef}
                ...
                {images?.map((image, index) => {
                    return <img alt='product' key={image?.url} src={image?.url} style={{ translate: `${-100 * currentImage}%`, transition: 'translate 700ms ease-in-out' }} className={`shrink-0 grow-0  object-cover w-full h-full`} onLoad={() => handleImageLoad(index)} />
                })}
